...
export default function PurchaseOrdersPage() {
  ...
  // Lee una cotización de proveedor con IA y precarga el formulario de "Nueva orden" con lo que
  // se logró leer — el usuario revisa/corrige antes de guardar, el guardado sigue siendo el mismo
  ...
  const scanQuotation = async (file) => {
    ...
    try {
      ...
      fd.append('file', file);
      const draft = await purchaseOrdersApi.scanQuotation(projectId, fd);
      setForm((f) => ({
        ...f,
      ...
      setShowForm(true);
      if (draft.extractionStatus === 'revisar') {
        setScanError(t('execution.purchaseOrders.scanNeedsReview'));
      }
    } catch (err) {
  ...
  return (
    <div>
      <Card title={t('execution.purchaseOrders.title')} actions={
        ...
        <Table columns={[t('suppliers.orders.table.number'), t('execution.purchaseOrders.table.supplier'), t('execution.purchaseOrders.table.date'), t('execution.purchaseOrders.table.status'), t('execution.purchaseOrders.table.items'), t('execution.purchaseOrders.table.total'), t('execution.purchaseOrders.table.paid'), t('execution.purchaseOrders.table.balance'), t('execution.purchaseOrders.table.expenses'), '']}>
          {orders.map((o) => (
            editingOrderId === o.id ? (
              ...
              <tr key={o.id} className="border-b border-gray-100">
                ...
                <td className="py-2 pr-3">{money(o.totals?.totalPaid ?? 0)}</td>
                <td className="py-2 pr-3">{money(o.totals?.balance ?? 0)}</td>
          ...
          {orders.length === 0 && <tr><td colSpan={10} className="py-3 text-center text-gray-400">{t('execution.purchaseOrders.empty')}</td></tr>}
        </Table>
        {isAdmin && (
...
function OrderDetail({ projectId, orderId, budgetItems, cashBoxes, onChange }) {
  ...
  const [saveEditReceipt, savingReceipt] = useSubmitGuard(async (itemId, receiptId) => {
    setError('');
    // Corregir una recepción ya usada por un proceso posterior (gasto o abonos) sigue permitido —
    // solo se advierte antes de confirmar, ver purchaseOrderController.updateReceipt.
    ...
    try {
      await purchaseOrdersApi.updateReceipt(projectId, orderId, itemId, receiptId, editReceiptForm);
  ...
  const [submitPayment, submittingPayment] = useSubmitGuard(async (e) => {
    ...
    try {
      ...
      await purchaseOrdersApi.addPayment(projectId, orderId, fd);
        ...
        {order.items?.map((it) => (
          ...
          {it.receipts?.length > 0 && (
            <tr className="border-b border-gray-100 bg-gray-50">
              <td colSpan={6} className="py-1 pr-3 pl-4">
